  ...
  <div class="wrapper">
    ...
    <a-tabs default-active-key="1" tabPosition="top" v-if="spinning == false">
      ...
      <div slot="tabBarExtraContent" v-if="spinning == false">
        <a-select
          label-in-value
          :default-value="{ key: '香格里拉' }"
          style="width: 120px"
          @change="handleChange"
          v-model:value="curValue"
        >
        <!-- :default-value="{ key: '香格里拉' }" -->
          <!-- <a-select-option value="香格里拉"> 香格里拉 </a-select-option>
          <a-select-option value="科索沃"> 科索沃 </a-select-option> -->
          <a-select-option
            :key="item.id"
...
export default {
  ...
  mounted() {
    // this.getUserInfo()
    this.getData(this.options[0].value);
  },
  methods: {
    moment,
    dataFormat(data) {
